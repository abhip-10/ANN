import gym
# Ensure legacy `Monitor` exists for packages expecting the old Gym API
try:
    from gym.wrappers import Monitor  # type: ignore
except Exception:
    try:
        # gymnasium compatibility: use RecordVideo if available
        from gymnasium.wrappers import RecordVideo as Monitor  # type: ignore
        setattr(gym.wrappers, 'Monitor', Monitor)
    except Exception:
        # Fallback no-op Monitor to avoid import errors in older packages
